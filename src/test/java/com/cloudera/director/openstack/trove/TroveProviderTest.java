...
//		when(credentials.buildTroveApi()).thenReturn(troveApi);
		...
		troveProvider = new TroveProvider(troveApi, resourceProviderConfiguration, credentials, DEFAULT_LOCALIZATION_CONTEXT);
	}
	...
	@Test
	public void testAllocate_PreExist() throws InterruptedException {
		Configured configuration = prepareTemplateConfig();
		TroveInstanceTemplate template = troveProvider.createResourceTemplate("trove-template", configuration, new HashMap<String, String>());
		...
		List<Instance> instanceList1 = ImmutableList.of();
		FluentIterable<Instance> instances1 = FluentIterable.from(instanceList1);
		when(instanceApi.list()).thenReturn(instances1);
		troveProvider.allocate(template, Lists.newArrayList(instanceNameSuffix), 1);
		
		List<Instance> instanceList2 = ImmutableList.of(instanceCreated);
		FluentIterable<Instance> instances2 = FluentIterable.from(instanceList2);
		when(instanceApi.list()).thenReturn(instances2);	
		troveProvider.allocate(template, Lists.newArrayList(instanceNameSuffix), 1);
		
		verify(instanceCreated, times(2)).getId();
		verify(instanceCreated, times(1)).getStatus();
		verify(instanceApi, times(0)).delete(instanceNameSuffix);
		assertThat(instanceCreated.getName()).isEqualTo(decoratedInstanceName);
		assertThat(instanceCreated.getId()).isEqualTo(instanceNameSuffix);
	...
	public void testFind() throws InterruptedException, UnknownHostException {
		...
		when(instanceCreated1.getName()).thenReturn(decoratedInstanceName1, decoratedInstanceName1, InetAddress.getLocalHost().getHostName());
		when(instanceCreated2.getName()).thenReturn(decoratedInstanceName2, decoratedInstanceName2, InetAddress.getLocalHost().getHostName());
		
		List<Instance> instanceList = ImmutableList.of(instanceCreated1, instanceCreated2);
		FluentIterable<Instance> instances = FluentIterable.from(instanceList);
		
		when(instanceApi.list()).thenReturn(instances);
		Collection<TroveInstance> existedInstances = troveProvider.find(template, Lists.newArrayList(instanceNameSuffix1, instanceNameSuffix2));
	...
	public void testFind_InvalidPrivateIp() throws InterruptedException, UnknownHostException {
		...
		String decoratedInstanceName = INSTANCE_NAME_PREFIX.unwrap().getDefaultValue() + "-" + instanceNameSuffix;
		Instance instanceCreated = mockInstance(TEST_FLAVOR_ID, Integer.parseInt(TEST_VOLUME_SIZE), decoratedInstanceName, instanceNameSuffix, Status.ACTIVE);
		List<Instance> instanceList = ImmutableList.of(instanceCreated);
		FluentIterable<Instance> instances = FluentIterable.from(instanceList);
		when(instanceApi.list()).thenReturn(instances);
		
		try {
			troveProvider.find(template, Lists.newArrayList(instanceNameSuffix));	
	...
	public void testFullCycle() throws InterruptedException, UnknownHostException {
		...
		FluentIterable<Instance> instances = FluentIterable.from(instanceList);
		when(instanceApi.list()).thenReturn(instances);
		when(instanceCreated.getName()).thenReturn(decoratedInstanceName, InetAddress.getLocalHost().getHostName(), decoratedInstanceName);
